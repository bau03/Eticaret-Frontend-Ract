...
import {addShoppingProductAsync} from "@internship/store/authentication";
import {useForm} from "react-hook-form";
import {toast, ToastContainer} from "react-toastify";
import {ShowcaseDetailResponse} from "../../../../../../libs/shared/api/src/lib/api/Auth";
...
export const ShowcaseDetailPage = () => {
  ...
  const [productsDetail, setProductsDetail] = useState<ShowcaseDetailResponse[]>();
            ...
            {productsDetail?.map(products => (
              <div key={products.id}>
                <Col className="flex-grow-0">
                  <StyledCard>
                    <StyledImageCard>
                      <StyledImage src={products.product.productImage}></StyledImage>
                    </StyledImageCard>
                    <div className="row mt-2">
                      <div className="col ml-1"><p>{products.product.categorys[0]?.categorys}</p></div>
                      <div className="col text-lg-right mr-1"> <p>
                        {Moment(products.product.timestap).format('DD MMM, YYYY')}
                      </p></div>
                    </div>
                    <StyledTitleCard>
                    ...
                    <StyledInfoCard>
                      <p>
                        <div className={"editor h-auto w-auto ml-4"} dangerouslySetInnerHTML={{ __html: products.product?.explanation}} />
                      </p>
                    </StyledInfoCard>
                    <StyledInfoCard>
                      <p className={"h-auto w-auto ml-4"}>
                        Ürün Fiyatı: {products.product.price}
                      </p>
                    </StyledInfoCard>
